...
2. Select `WoWCombatLog.txt`, another `.txt`/`.log` combat log, or a `.zip` containing the log.
3. Confirm the optional uploader/guild and realm fields.
4. Keep the tab open while the upload, quick classification, full parse, and save stages complete.
5. Open the generated raid-session link.
