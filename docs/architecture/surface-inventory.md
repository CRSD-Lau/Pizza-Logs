...
When configured, a separate Railway cron service runs `node scripts/notification-worker.mjs` every five minutes and exits. Newly stored uploads enqueue one transactional `UPLOAD_COMPLETED` job; the worker also deduplicates one `DAILY_DIGEST` job per prior UTC day. Jobs use leases, bounded retries and provider idempotency. This is an owner-only operations path, not a public API or durable parse queue. It stores aggregate digest results and sanitized upload summaries, never raw logs, request IPs or user agents.

Egress: web -> internal parser, PostgreSQL, armory.warmane.com and Warmane API/CDN content. Notification worker -> PostgreSQL, Railway GraphQL and Resend. Browser may load permitted Warmane/CDN and Wowhead images/viewer assets under the existing CSP. GitHub, npm and PyPI are build/maintenance dependencies; normal report rendering does not call the UwU reference or GitHub.
