...
import { buildUserscript, PIZZA_LOGS_ORIGIN, USERSCRIPT_URL } from "../lib/armory-gear-client-scripts";

const userscript = buildUserscript();

assert.equal(PIZZA_LOGS_ORIGIN, "https://pizza-logs-production.up.railway.app");
assert.equal(USERSCRIPT_URL, `${PIZZA_LOGS_ORIGIN}/api/admin/armory-gear/userscript`);
