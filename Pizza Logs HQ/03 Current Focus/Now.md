...
**Upload-page cinematic intro is implemented on `codex/upload-cinematic-intro` and has a stronger fan-art-style pass.** The previous global Frozen Logbook intro has been replaced with `UploadCinematicIntro`, mounted only from the current public Upload route (`/`). It uses original CSS/SVG frozen-fantasy visuals with a distant back-facing approach, aggro detail reveal, larger frozen warlord silhouette, ice spires, frost-fire, glowing rune core, blade swing, crack impact, and page reveal. It plays for `3800ms`, has visible `Skip` and Escape-key skip, fully unmounts after skip/completion, and does not use localStorage, external media, audio, video, WebGL, Three.js, framer-motion, copyrighted assets, or new dependencies.
