...
**Upload-page cinematic intro is implemented on `codex/upload-cinematic-intro` and now has a less blocky painted-warrior SVG pass.** The previous global Frozen Logbook intro has been replaced with `UploadCinematicIntro`, mounted only from the current public Upload route (`/`). It uses original CSS/SVG frozen-fantasy visuals with a distant back-facing approach, aggro detail reveal, layered cloak, roughened painted edge filter, larger jagged ice crown, heavier shoulders, mask/cheek plates, fur/ice mantle, glowing rune blade/core/veins, blade swing, crack impact, and page reveal. It plays for `3800ms`, has visible `Skip` and Escape-key skip, fully unmounts after skip/completion, and does not use localStorage, external media, audio, video, WebGL, Three.js, framer-motion, copyrighted assets, or new dependencies.
