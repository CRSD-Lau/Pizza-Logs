  ...
  return (
    <header className={cn("flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between", className)}>
      <div className="min-w-0">
        ...
        <h1 className="heading-cinzel text-2xl font-bold text-heading sm:text-3xl">{title}</h1>
    ...
    <section id={id} className={cn("page-section scroll-mt-36", className)}>
      {(title || description || action) && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            {title && <h2 className="heading-cinzel text-lg font-semibold text-heading">{title}</h2>}
